        ...
        for corr in corruption_types:
            for level in corruption_levels:
                for img_name in class_images:
                    ...
                    ori_img = Image.open(
                        os.path.join(
                            ORIGINAL_DATA_DIR, c_name, img_name.split(".")[0] + ".JPEG"
                        )
                    ).convert("RGB")
                    # ori_img = transform(ori_img)
                    # Get the mask image
                    mask_img = np.load(os.path.join(MASK_DIR, c_name, img_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/data/sayanta/datasets/OCD/imagenet-9/9_made_cs",
