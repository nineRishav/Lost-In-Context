...
def main(args: Namespace, variant):
    ...
    output_txt_file_path = os.path.join(
        output_base_path, f"accuracy/noises/A4-accuracy-{variant}.txt"
    )
    output_json_file_path = os.path.join(
        output_base_path, f"accuracy/noises/A4-accuracy-{variant}.json"
    )

    # Create the accuracy directory if it does not exist
    accuracy_directory = os.path.dirname(output_txt_file_path)
    ...
    output_json_images_file_path = os.path.join(
        output_path_directory, f"noises/A4-accuracy-image_paths-wrong-ooc-{variant}.json"
    )
...
if __name__ == "__main__":
    ...
    for variant in variants:
        args.data_path = f"../../data/imagenet-9/noises/{variant}"
        main(args, variant)
...
"""
Work : Calculates the accuracy of all noise variant of the model, and saves their paths in a json file.
